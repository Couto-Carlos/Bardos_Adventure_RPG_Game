...
public class GuitarTunerActivity extends AppCompatActivity implements TunerPitchToggleView,
    ...
    private static GuitarTunerActivity context;

    public static GuitarTunerActivity getInstance() {
        if (context == null) {
          //  context = new GuitarTunerActivity();
        }
        return context;
    }


    public static void SendFreq (){
    ...
    @Override
    public void showGuitarTuner() throws InterruptedException {
        new Thread(new ThreadToFrequency()).start();
    ...
    private  void showFraquency() throws InterruptedException {
        for(int i = 0; i>100; i++){
            Thread.sleep(1000);
          //  BluetoothManager.write(GuitarTuner.frequency, this);
        }
